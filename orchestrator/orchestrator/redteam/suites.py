"""Scan suites derived from the probe library's existing standards mappings.
...
SUITE_IDS: tuple[str, ...] = tuple(SUITE_MAPPING_FIELDS)
...
def derive_suites(probes: Iterable) -> dict[str, list[str]]:
    ...
    """
    derived: dict[str, list[str]] = {suite_id: [] for suite_id in SUITE_MAPPING_FIELDS}
    for probe in probes:
