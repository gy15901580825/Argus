...
def _extract_first_json_object(text: str) -> str:
	...
	gpt-5.4-mini in reasoning mode occasionally appends a stray newline or
	`<think>...</think>` block after the structured-output payload. Pydantic's
	model_validate_json is strict and rejects this with `trailing characters at
	line N column M`. Use json.JSONDecoder.raw_decode to find the prefix that
	actually parses, then return that slice. If no valid JSON is present,
	return *text* unchanged so the original error surfaces upstream.
	"""
